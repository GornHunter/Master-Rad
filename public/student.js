...
const studentSubjectPointsFormDOM = document.querySelector(
  ".studentSubjectPoints-form"
);

let id = "";

document.querySelector(".load").onchange = () => {
  try {
    const file = document.querySelector(".load").files[0];
    ...
    reader.onload = async function () {
      ...
      for (let i = 0; i < students.students.length; i++) {
        await axios.post("/api/v1/students", {
          ...
          index: students.students[i].index,
        });
      }
      showStudents();
    };
  ...
  try {
    ...
    if (students.length < 1) {
      studentsDOM.innerHTML =
        '<h5 class="empty-list">Lista studenata je prazna</h5>';
      return;
    }
  ...
  if (element.parentElement.classList.contains("edit-link")) {
    ...
    try {
      const {
        ...
        `/api/v1/students/${element.parentElement.dataset.id}`
      );
      const { _id: studentID, firstName, lastName, index } = student;
    ...
    try {
      if (document.querySelector(".schoolYear-div") == null) addElement();

      document.getElementById("subj").innerHTML = "";
      document.getElementById("stuSub-form").className = "studentSubject-form";
      document.getElementById("studentSubjectModalLabel").innerHTML =
        "Dodeljivanje predmeta studentu";
      document.getElementById("btnStudentSubject").innerHTML =
        "Dodeli predmete studentu";
      ...
      const {
        data: { studentSubject },
      } = await axios.get(`/api/v1/studentSubjects/${id}&`);
      ...
      for (let i = 0; i < subjects.length; i++) {
        if (subjects[i]._id !== "0")
          items.push(
            `<option value="${subjects[i]._id}">${subjects[i].name}</option>`
          );
        else
          items.push(
            `<option value="${subjects[i]._id}" disabled>${subjects[i].name}</option>`
          );
      }
    ...
    element.parentElement.classList.contains("studentSubjectPoints-link")
  ) {
    ...
    const {
      data: { student },
    } = await axios.get(`/api/v1/students/${element.parentElement.dataset.id}`);
    const { _id: studentID, firstName, lastName } = student;
    document.getElementById(
      "wholeNamePoints"
    ).innerHTML = `${firstName} ${lastName}`;
    ...
    for (let i = 0; i < studentSubject.length; i++) {
      const {
        data: { subject },
      } = await axios.get(`/api/v1/subjects/${studentSubject[i].subject_id}`);
      items.push(
        `<option value="${subject._id}" onclick="showPoints()">${subject.name}</option>`
      );
    }

    document.getElementById("subjPoints").innerHTML = items;
  } else if (
    element.parentElement.classList.contains("studentSubjectDelete-link")
  ) {
    try {
      if (document.querySelector(".schoolYear-div") != null)
        studentSubjectFormDOM.removeChild(
          document.querySelector(".schoolYear-div")
        );

      document.getElementById("subj").innerHTML = "";
      document.getElementById("stuSub-form").className =
        "studentSubjectDelete-form";
      document.getElementById("studentSubjectModalLabel").innerHTML =
        "Uklanjanje dodeljenih predmeta";
      document.getElementById("btnStudentSubject").innerHTML =
        "Ukloni dodeljenje predmete";
    ...
    if (
      element.classList.contains("fullName") ||
      element.classList.contains("index-color")
    ) {
      let tmp =
      ...
      try {
        const {
          data: { student },
        } = await axios.get(`/api/v1/students/${tmp}`);
        const { _id: studentID, firstName, lastName, index } = student;

        const {
          data: { studentSubject },
        } = await axios.get(`/api/v1/studentSubjects/${tmp}&`);

        document.getElementById(
          "informationModalLabel"
        ).innerHTML = `Informacije o studentu ${firstName} ${lastName}`;
        let sum = 0;
        ...
        if (studentSubject.length == 0)
          document.querySelector(
            ".info"
          ).innerHTML = `<h5>Student ${firstName} ${lastName} sa brojem indeksa ${index} nema predmeta koje slusa.</h5>`;
        else {
          document.querySelector(
            ".info"
          ).innerHTML = `<h5>Student ${firstName} ${lastName} sa brojem indeksa ${index} slusa sledece predmete:</h5>`;
          for (let i = 0; i < studentSubject.length; i++) {
            sum = 0;
            const {
              data: { subject },
            } = await axios.get(
              `/api/v1/subjects/${studentSubject[i].subject_id}`
            );
            document.querySelector(
              ".info"
            ).innerHTML += `<h4><ul><li><strong>${subject.name}</strong>&nbsp;(${studentSubject[i].school_year})</li><ul id="cat${subject._id}"></ul></ul></h4>`;
            for (let j = 0; j < subject.categories.length; j++) {
              ...
              document.getElementById(
                `cat${subject._id}`
              ).innerHTML += `<h5><ul><li>${subject.categories[j]} - &nbsp;<strong>${tmp1}</strong> poen/a</li></ul></h5>`;
            }
            document.querySelector(
              ".info"
            ).innerHTML += `<h5>Ukupan broj poena - <strong>${sum}</strong></h5>`;
          }
        }
      } catch (error) {
        console.log(error);
...
studentFormDOM.addEventListener("submit", async (e) => {
  ...
  try {
    if (document.getElementById("stu-form").className == "student-form") {
      ...
      const {
        data: { student },
      } = await axios.patch(`/api/v1/students/${id}`, {
        firstName,
        lastName,
        index,
      });
    ...
    if (firstName == "")
      studentFormAlertDOM.innerHTML +=
        error.response.data.msg.errors.firstName.message + "<br />";

    if (lastName == "")
      studentFormAlertDOM.innerHTML +=
        error.response.data.msg.errors.lastName.message + "<br />";

    if (index == "")
      studentFormAlertDOM.innerHTML +=
        error.response.data.msg.errors.index.message;
...
studentSubjectFormDOM.addEventListener("submit", async (e) => {
  ...
  const student_id = id;
  const subject_id =
    document.getElementById("subj").options[
      document.getElementById("subj").selectedIndex
    ].value;
  let school_year = "";
  ...
  try {
    if (
      ...
      await axios.post("/api/v1/studentSubjects", {
        student_id,
        subject_id,
        school_year,
      });

      document.querySelector(".studentSubjectForm-alert").style.display =
        "block";
      document.querySelector(".studentSubjectForm-alert").textContent =
        "Predmet uspesno dodeljen studentu";
      document
        .querySelector(".studentSubjectForm-alert")
    ...
    if (subject_id == "")
      document.querySelector(".studentSubjectForm-alert").innerHTML +=
        error.response.data.msg.errors.subject_id.message + "<br />";
    ...
    if (
      ...
      document.querySelector(".studentSubjectForm-alert").innerHTML +=
        error.response.data.msg.errors.school_year.message;

    if (
      school_year != "" &&
      error.response.data.msg.errors.school_year != undefined
    )
      document.querySelector(".studentSubjectForm-alert").innerHTML +=
        error.response.data.msg.errors.school_year.message;
...
studentSubjectPointsFormDOM.addEventListener("submit", async (e) => {
  ...
  const student_id = id;
  const subject_id =
    document.getElementById("subjPoints").options[
      document.getElementById("subjPoints").selectedIndex
    ].value;
  ...
  try {
    if (num > 0) {
      ...
      if (pointsStr == "") points = [];
      else points = Number(pointsStr);
    }

    if (subject_id == "") {
      document.querySelector(".studentSubjectPointsForm-alert").style.display =
        "block";
      document.querySelector(".studentSubjectPointsForm-alert").innerHTML = "";

      document.querySelector(".studentSubjectPointsForm-alert").innerHTML +=
        "Predmet mora biti odabran";

      document
        .querySelector(".studentSubjectPointsForm-alert")
        .classList.remove("text-success");
      document
        .querySelector(".studentSubjectPointsForm-alert")
        .classList.add("text-danger");
      ...
      setTimeout(() => {
        document.querySelector(
          ".studentSubjectPointsForm-alert"
        ).style.display = "none";
      }, 2000);
    ...
    const {
      ...
      `/api/v1/studentSubjects/${student_id}&${subject_id}`,
      {
        points,
      }
    );
    ...
    document.querySelector(".studentSubjectPointsForm-alert").style.display =
      "block";
    document.querySelector(".studentSubjectPointsForm-alert").textContent =
      "Uspesno uneti poeni";
    document
      .querySelector(".studentSubjectPointsForm-alert")
      .classList.remove("text-danger");
